...
import { wheelData } from "./DonutWheelData";
  ...
  export const getDayOfYear = (date) =>  {
    ...
    export const getXY = (radius, angle, origin) => {
      ...
      export const drawArc = (innerRadius: number, outerradius: number, item, title:string, colour: string) => {
        const arc = d3.arc()
        return {
          arcSvg: arc({
      ...
      export const drawText =(arr:any[], rotate: number, svgEl) =>{
        arr.forEach((coord, index) => {
            svgEl
              ...
              .text(coord.title)
              .attr("transform", `rotate(${(rotate + ((360 / arr.length)) * (arr.length - index))}, ${coord.coords.x}, ${coord.coords.y})`)
          });
      }
      ...
      export const populateArr =( arr, divider: number, radius: number, text?:string) => {
      ...
      export const populateMonthsArrUpper =(arr, divider: number, angel: number, text:string[]) => {
        let index = 0;
        let realIndex = 11;
      for (let i = 0; i <= 360; i += (360 / divider)) {
          index++;
    ...
    export const populateMonthsArrLower =(arr, divider: number, angel: number, text:string[]) => {
        let index = 0;
        let realIndex = 11;
      for (let i = 0; i <= 360; i += (360 / divider)) {
          index++;
    ...
    export const populateWeeksUpper =( arr, divider: number, angel: number, text:string[]) => {
        let realIndex = 26;
        for (let i = 0; i < 360; i += 360 / divider) {
          arr.push({
            title: text[realIndex] || '',
      ...
      export const populateWeeksLower =( arr, divider: number, angel:number, text:string[]) => {
        let realIndex = 26;
        for (let i = 0; i < 360; i += 360 / divider) {
          arr.push({
            title: text[realIndex] || '',
      ...
      export const populateCategoryUpper =( arr, divider: number, angel:number, radius: number, text:string) => {
        for (let i = 0; i < 360; i += 360 / divider) {
          arr.push({
      ...
      export const populateCategoryLower = ( arr, divider: number, angel:number, radius: number, text:string) => {
        for (let i = 0; i < 360; i += 360 / divider) {
          arr.push({
            title: i >= 90 && i <= 270 ? text : '',
            coords: getXY(radius, i - angel, 500),
